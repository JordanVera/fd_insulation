...
import { PhoneIcon, MailIcon, MapPinIcon, ClockIcon, ShieldCheckIcon } from 'lucide-react';
  ...
  return (
    <footer className="border-t border-border bg-muted/40 dark:bg-card transition-colors">
      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-12 md:grid-cols-4 md:px-6">
        ...
        <div className="md:col-span-1">
          ...
          <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
            Houston&apos;s leading attic and spray foam insulation contractor — improving
            comfort and energy efficiency for 35+ years.
          </p>
          <div className="mt-5 space-y-2.5 text-sm">
            <div className="space-y-1">
              <a href={site.phones.houston.href} className="flex items-center gap-2 font-semibold transition-colors hover:text-primary">
                <PhoneIcon className="size-4 shrink-0 text-primary" />
                Houston: {site.phones.houston.display}
              </a>
              <a href={site.phones.dfw.href} className="flex items-center gap-2 font-medium text-muted-foreground transition-colors hover:text-foreground pl-6">
                DFW: {site.phones.dfw.display}
              </a>
            </div>
            <a href={`mailto:${site.email}`} className="flex items-center gap-2 text-muted-foreground transition-colors hover:text-foreground">
            ...
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span className="text-xs">{site.address.city}, {site.address.state}</span>
            </div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {services.map((s) => (
              <li key={s.slug} className="text-muted-foreground">{s.name}</li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {site.serviceAreas.map((area) => (
              <li key={area} className="flex items-center gap-1.5 text-sm text-muted-foreground">
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-2 px-4 py-5 text-xs text-muted-foreground md:flex-row md:justify-between md:px-6">
        <p>© {new Date().getFullYear()} First Defense Insulation. All rights reserved.</p>
